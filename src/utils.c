...
void log_print(const char *level, const char *fmt, ...) {
    ...
    struct timeval tv;
    gettimeofday(&tv, NULL);
#if defined(LOG_HUMAN_TIME) && LOG_HUMAN_TIME
    // Human readable local time (system calls only)
    struct tm tm_info;
    char ts_buf[64];
    ...
    printf("[%s] [%s] %s\n", ts_buf, level, buffer);
#else
    printf("[%ld.%06ld] [%s] %s\n", (long)tv.tv_sec, (long)tv.tv_usec, level, buffer);
#endif

    pthread_mutex_lock(&g_log_mutex);
    if (!g_log_file) {
        const char *path = (g_cfg.log.file[0] ? g_cfg.log.file : "/tmp/gb28181_wvp.log");
...
#else
        fprintf(g_log_file, "[%ld.%06ld] [%s] %s\n", (long)tv2.tv_sec, (long)tv2.tv_usec, level, buffer);
#endif
        fflush(g_log_file);
    }
